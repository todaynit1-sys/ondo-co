...
<section class="sec sec-mist">
  <div class="wrap">
    ...
    <p class="cmp-note rv" style="margin-top:24px">
      바로 위 네 화면은 실제 고객 사이트가 아니라 <b>구성 순서를 보여 주는 예시</b>입니다.
      실제로 배포된 사이트는 이 페이지 맨 위에 있습니다. 나머지 업종은 데모가 준비되는 대로 주소로 걸어 두겠습니다.
    </p>
  </div>
</section>
"""
...
WEB_PLANS = "".join([
 plan("베이직", "일단 주소부터 필요한 경우", "5", "만원",
      ["원페이지 3~4섹션", "모바일 반응형", "전화 바로걸기 버튼", "카톡 문의 버튼", "카톡 공유 미리보기", "배포 주소 + 소스", "월 호스팅비 0원", "무료 수정 1회"], "베이직 문의", K, was="7만원"),
 plan("스탠다드", "손님을 실제로 받는 가게라면", "10", "만원",
      ["5~7섹션 · 베이직 전부 포함", "갤러리 · 메뉴 · 전후 비교", "네이버 지도 · 오시는 길", "사진 보정 · 웹 최적화", "검색 등록 기본 세팅", "월 호스팅비 0원", "무료 수정 1회"],
      "스탠다드 문의", K, best=True, tag="추천 구성", was="15만원"),
 plan("프리미엄", "페이지를 더 나눠야 하는 경우", "20", "만원~",
      ["스탠다드 전부 포함", "서브페이지 1~2개", "업종별 맞춤 기능 협의", "구성 상담 후 확정 견적", "무료 수정 2회"], "프리미엄 문의", K, was="28만원~"),
])
CLIP_PLANS = "".join([
...
AI_PLANS = "".join([
 plan("제작", "만들 것이 이미 정해진 경우", "8", "만원",
      ["정해 오신 주제로 제작", "작동하는 결과물 1개", "배포 주소 + 소스 전체", "구조 설명 문서",
       "이력서·자소서 문장 + 면접 답변 뼈대", "무료 수정 2회"], "제작 문의", K, was="12만원"),
 plan("기획 + 제작", "뭘 만들지부터 정해야 한다면", "15", "만원",
      ["공고·직무 분석", "주제 후보 2~3개 제안", "제작 구성 전부 포함", "공고 1건 맞춤 조정",
       "이력서·자소서 문장 + 면접 답변 뼈대"],
      "기획 + 제작 문의", K, best=True, tag="추천 구성", was="22만원"),
 plan("＋ 바이브코딩 강의", "다음 결과물은 직접 만들려면", "25", "만원~",
      ["앞 구성 전부 포함", "제작 강의 6강 (녹화 영상)", "실습용 예제 파일", "2주간 카톡 질문", "두 번째 결과물 점검 1회"], "강의 포함 문의", K, was="36만원~"),
])

_TAIL = "<b>확정</b> · 표시 금액은 상담에서 구성을 확인한 뒤 확정합니다. 시작한 다음에 올리지 않습니다"
...
PANELS = (
 panel("pp-web", "pt-web", WEB_PLANS,
       ...
       "<b>연락 방법</b> · 기본은 전화 바로걸기 + 카톡 문의 버튼입니다. 이메일 문의폼은 옵션으로 붙일 수 있습니다<br>"
...
body = (body
  ...
  .replace("__FAQ__", faq([
    ...
    ("계약금이나 선입금이 있나요?",
     "꼭 필요한 건 아닙니다. 규모가 있는 작업만 절반씩 나눠서 진행하고, 그 외에는 완성본을 보신 뒤에 결제하셔도 됩니다."),
    ("결제는 어떻게 하나요?",
     "크몽으로 오시면 크몽 결제 절차를 그대로 따르고, 카톡으로 직접 오시면 계좌 이체로 진행합니다. 소액은 완성 후 결제, 규모가 있는 작업은 절반씩 나누는 방식 중에서 편하신 쪽으로 정하시면 됩니다."),
    ("월 유지비가 정말 없나요?",
...
page("pricing/index.html",
     ...
     "반응형 홈페이지 5만원부터, AI 홍보 영상 30초 20만원·60초 40만원, 취업용 AI 결과물 8만원부터. 포함 사항과 추가 비용을 미리 구분해 적어 두었습니다.",
     body, P, "pricing/")
