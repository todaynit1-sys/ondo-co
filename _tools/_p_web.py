...
<section class="phead">
  <div class="wrap phead-in">
    ...
    <p class="phead-lede">네이버·구글에서 찾았을 때 <b>뭘 얼마에 어떻게 해주는지</b> 한 화면에 보이는 홈페이지.
      전부 스마트폰 기준으로 먼저 만들고, 완성되면 바로 열리는 주소로 올려 드립니다.
      <b>만든 다음 매달 나가는 돈은 0원</b>입니다.</p>
...
<section class="sec sec-mist" id="price">
  <div class="wrap">
    ...
    <p class="lede rv">작업 중에 늘어나는 비용이 없도록, 포함되는 것과 아닌 것을 미리 구분해 두었습니다.
      <b>세 구성 모두 만들고 난 뒤 매달 나가는 돈이 0원</b>입니다. 사이트를 띄워 두는 비용을 저희가 안 받습니다.</p>
    <div class="plans" style="margin-top:38px">__PLANS__</div>
    <div class="plan-note rv">
      <b>옵션</b> · 커스텀 도메인 연결 +2~3만원(도메인 값은 본인 명의로 별도 구입) / 섹션 추가 건당 1~2만원 / 문의폼 연결 +1~2만원<br>
      <b>수정</b> · 표기된 횟수 포함. 이후는 건당 1~2만원으로 안내드립니다<br>
      <b>연락 방법</b> · 기본은 <b>전화 바로걸기 + 카톡 문의 버튼</b>입니다. 이메일로 받는 문의폼은 옵션으로 붙일 수 있습니다<br>
      <b>확정</b> · 표시 금액은 상담에서 구성을 확인한 뒤 확정합니다. 시작한 다음에 올리지 않습니다
    </div>
  </div>
</section>
...
PLANS = "".join([
 plan("베이직", "일단 주소부터 필요한 경우", "5", "만원",
      ["원페이지 3~4섹션 (히어로·소개·연락처)", "모바일 반응형", "전화 바로걸기 버튼",
       "카톡 문의 버튼", "카톡 공유 미리보기", "배포 주소 + 소스 파일", "월 호스팅비 0원", "무료 수정 1회"],
      "베이직 문의", K, was="7만원"),
 plan("스탠다드", "손님을 실제로 받는 가게라면", "10", "만원",
      ["5~7섹션 · 베이직 전부 포함", "갤러리 · 메뉴 · 시공 전후 비교", "네이버 지도 · 오시는 길",
       "사진 보정 · 웹 최적화", "검색 등록 기본 세팅", "월 호스팅비 0원", "무료 수정 1회"],
      "스탠다드 문의", K, best=True, tag="추천 구성", was="15만원"),
 plan("프리미엄", "페이지를 더 나눠야 하는 경우", "20", "만원~",
      ["스탠다드 전부 포함", "서브페이지 1~2개", "업종별 맞춤 기능 협의",
       "구성 상담 후 확정 견적", "무료 수정 2회"],
      "프리미엄 문의", K, was="28만원~"),
])
...
body = (body
  ...
  .replace("__FAQ__", faq([
     ...
     ("온라인 결제나 예약 기능도 되나요?",
      "결제·회원가입·실시간 예약처럼 서버를 계속 관리해야 하는 기능은 <b>이 서비스 범위 밖</b>입니다. 대신 카톡 문의 버튼, 전화 바로걸기, 구글폼 예약 링크를 연결해 드리는 방식으로 대부분 해결됩니다. 필요하신 흐름을 말씀해 주시면 가능한 방법을 알려 드리겠습니다."),
     ("사진이 없는데 만들 수 있나요?",
     ...
     ("손님 문의는 어떻게 받나요? 이메일로도 오나요?",
      "기본은 <b>전화 바로걸기</b>와 <b>카톡 문의 버튼</b>입니다. 손님이 누르면 바로 통화나 대화창으로 넘어가서 사장님이 따로 관리할 게 없습니다. 이메일로 받는 문의폼도 붙일 수 있는데, 사장님 명의로 계정을 하나 만들어야 하고 무료로 받을 수 있는 건수에 한도가 있습니다. 그래서 옵션으로 두었습니다."),
     ("만든 사이트는 제 것이 되나요?",
      "네. 완성된 소스 파일을 함께 드립니다. 나중에 다른 곳으로 옮기시거나 직접 고치셔도 됩니다."),
     ("수정은 몇 번까지 되나요?",
      "베이직·스탠다드는 무료 수정 <b>1회</b>, 프리미엄은 2회가 포함됩니다. 문구 교체, 사진 교체, 순서 변경 모두 가능합니다. 그 이후는 건당 1~2만원 선으로 안내드립니다. <b>고칠 곳을 한 번에 모아서</b> 주시면 한 번에 다 반영되니 훨씬 유리합니다."),
  ])))
...
LD = {"@context": "https://schema.org", "@type": "Service", "name": "온도 웹",
      ...
      "description": "소상공인을 위한 모바일 우선 반응형 홈페이지 제작. 전화 바로걸기·카톡 문의·네이버 지도 포함, 배포된 주소까지 납품하고 월 호스팅비는 0원.",
      "offers": {"@type": "AggregateOffer", "priceCurrency": "KRW", "lowPrice": "50000", "highPrice": "200000"}}

page("web/index.html",
     "온도 웹 — 소상공인 반응형 홈페이지 제작 | 온도컴퍼니",
     "가게 이름을 검색했을 때 나올 자리를 만듭니다. 모바일 우선 반응형, 전화 바로걸기·카톡 문의·네이버 지도 기본 포함. 5만원부터, 3~5일, 배포 주소까지 납품하고 월 유지비 0원.",
